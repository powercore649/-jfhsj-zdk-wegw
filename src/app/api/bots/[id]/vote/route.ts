...
export async function POST(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const lastVote = await prisma.vote.findFirst({
    where: { botId: params.id, userId, createdAt: { gt: cutoff } },
  ...
  await prisma.vote.create({ data: { botId: params.id, userId } });
  const count = await prisma.vote.count({ where: { botId: params.id } });
